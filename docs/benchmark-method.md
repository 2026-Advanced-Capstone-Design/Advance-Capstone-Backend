...
| 2026-07-11 | 단일 t2.micro (Phase 7~9 반영) | 100 | true (delay 5s) | 0.00% | 17 (p95 24 / p99 29) | 33.5 |
| 2026-07-11 | 단일 t2.micro (Phase 7~9 반영) | 500 | true (delay 5s) | **0.00%** | 16 (p95 23 / p99 30) | 83.3 |

\* 1000명/2노드 평균 응답 27.7s는 **대기열 포화 구간**(수용량 초과 시 지연 급증)을 의미. TPS는 유지되나 지연이 튀는 전형적 형태.

**해석 요지**
- 단일 노드 한계 = 100명 안정 / 500명에서 에러율 급증. (2026-05-21 기준)
...
- **(2026-07-11, Phase 8 검증)** 트랜잭션 경계 정리(사례 6) 후 동일 단일 노드에서 **500명 에러 8.58% → 0.00%**, 평균 103ms → 16ms. 부하 내내 `hikaricp_connections_pending=0`(기본 pool 10). AI 지연은 `MOCK_DELAY_SECONDS=5`로 재현했으며, 비동기 202 응답 구조라 AI 지연이 제출 응답시간에 전가되지 않음을 확인. 결함 자체의 before/after는 로컬 재현으로 입증 — **before: active 10(상한) ~135초 고정·pending 최대 98 / after: pending 전 구간 0**. 원본 데이터·차트: [`benchmarks/phase8-hikaricp/`](../benchmarks/phase8-hikaricp/README.md)
